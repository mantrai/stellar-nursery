...
export default class AridPlanetGen extends BasePlanetGen implements IPlanetGen {
    ...
    run(workObj: PlanetTypeWorker): Orbit<IPlanet> {
        ...
        if (roll <= 6) {
            ...
            stats.description = 'These are Arid worlds with less than 30% surface water coverage, and lacking any kind of plate tectonics. Most of the planet\'s water is locked up within its biomass, which aids in maintaining global habitability.';
            water = true;
        } else if (roll <= 8) {
            stats.chemistry.push(Chemicals.Ammonia);
            stats.planetType = 'Saganian';
            stats.description = 'These are ammonia equivalents of Darwinian worlds, the planet\'s water being mixed with liquid ammonia, the biomass fully adapted and dependent on its presence.';
            ageMod += 1;
        } else {
            stats.chemistry.push(Chemicals.Methane);
            stats.planetType = 'Asimovian';
            stats.description = 'These are methane equivalents of Darwinian worlds, the planet\'s water being mixed with liquid methane, the biomass fully adapted and dependent on its presence.  These worlds are found around the dimmer M-type dwarf stars..';
            ageMod += 3;
        }
